...
void getservers_command::handle_command(const network::address& target, const std::string_view& data)
{
    ...
    size_t server_count = prepared_servers.size();
	std::string response{};
	...
	while (!prepared_servers.empty())
	{
        ...
        if (game_type == game_type::h2m) response.push_back('\0');
	...
	console::log("Sent %zu servers in %zu parts for game %s:\t%s", server_count, packet_count, game.data(), target.to_string().c_str());
}
